							...
							<td colspan="2" align="left">
								<c:choose>
									<c:when test="${menuItem=='receive'}">
										보낸사람 : ${dto.senderName}
												<c:if test="${sessionScope.member.membership>50}">(${dto.senderId})</c:if>  
									</c:when>
									<c:when test="${menuItem=='send'}">
										받는사람 : ${dto.receiverName}
										<c:if test="${sessionScope.member.membership>50}">(${dto.receiverId})</c:if>  
									</c:when>
								</c:choose>
							</td>
						</tr>
						...
						<tr>
							<td colspan="2" class="ellipsis">
								...
								<c:if test="${not empty prevDto}">
									<span><a href="${pageContext.request.contextPath}/note/${menuItem}/article?${query}&num=${prevDto.num}">${prevDto.content}</a></span>
								</c:if>
							</td>
						</tr>
						<tr>
			...
			<div class="modal-body">
				<form name="replyForm" method="post">
					<div class="row reply-form m-1">
						...
						<input type="hidden" name="receivers" value="${dto.senderIdx}">
					</div>
				</form>
			</div>
